...
namespace DemoblazeUiTAF.DragAndDropGlobalsqaTests.Pages
{
    internal class MainPage : PageBase
    {
       ...
       public bool IsElementInTrash(string photoName)
        ...
        public ImageWidget[] GetAllPhotoElements()
        {
            IReadOnlyCollection<IWebElement> photoWebElements = Driver.FindElements(By.XPath("//ul[@id='gallery']/li"));
            ImageWidget[] imageWidgets = photoWebElements.Select(i => new ImageWidget(i)).ToArray();
            return imageWidgets;
        }

        public void WaitUntilImageIsZoomed(string imageSearchKey)
        {
            By searchLocator = By.XPath($"//div[@role ='dialog']//img[@src = '{imageSearchKey}']");
            wait.Until(ExpectedConditions.ElementIsVisible(searchLocator)); 
        }
    }
}
